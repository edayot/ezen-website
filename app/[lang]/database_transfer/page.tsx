        ...
        q.docs.forEach((doc) => {
            ...
            if (!data.image) {
                addDoc(newRef, data).then((newDocRef) => {
                    console.log(`Article without image added, oldRef : ${doc.id}, newRef : ${newDocRef.id}`)
                })
                return
            }
            ...
            img.onload = () => {
                ...
                const new_filename = data.image_filename.split(".")[0] + "_" + timestamp + ".png";
                    ...
                    (snapshot) => {
                        getDownloadURL(snapshot.ref).then((url) => {
                            addDoc(newRef, {
                                ...
                                console.log(`Article added, oldRef : ${doc.id}, newRef : ${newDocRef.id}`)
                            })
                        });
                    })
                    .catch(
...
export default function Home() {
    ...
    return <Button onClick={Transfer}>
        Transfer database
    </Button>
}
